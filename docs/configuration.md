...
| `LONG_TERM_MEMORY_MODEL` | `DEFAULT_LLM_MODEL` | LLM used by mem0 to extract memories; must be supported by the configured LLM endpoint |
...
When enabled, the Agent can search, browse, read, import, write, inspect, and delete resources under
OpenViking's `viking://resources` or user-resource namespaces. Admin, session, memory, skill, and
snapshot APIs are intentionally not exposed as Agent tools.
